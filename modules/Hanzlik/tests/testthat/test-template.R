...
#    `functionName` is a function's name in your module (e.g., `HanzlikEvent1`).
# 2. Copy this file to the tests folder (i.e., `/Users/stevec/Dropbox/Courses/7043H16/Lab/Harvesting/Hanzlik/tests/testthat`).

# 3. Modify the test description based on the content you are testing:
test_that("test Event1 and Event2.", {
  module <- list("Hanzlik")
  path <- list(modulePath = "/Users/stevec/Dropbox/Courses/7043H16/Lab/Harvesting",
               outputPath = file.path(tempdir(), "outputs"))
  parameters <- list(
    #.progress = list(type = "graphical", interval = 1),
    .globals = list(verbose = FALSE),
    Hanzlik = list(.saveInitialTime = NA)
  )
  ...
  if (exists("HanzlikEvent1", envir = .GlobalEnv)) {
    simOutput <- HanzlikEvent1(mySim)
  } else {
    simOutput <- mySim$HanzlikEvent1(mySim)
  }
  ...
  if (exists("HanzlikEvent2", envir = .GlobalEnv)) {
    simOutput <- HanzlikEvent2(mySim)
  } else {
    simOutput <- mySim$HanzlikEvent2(mySim)
  }
